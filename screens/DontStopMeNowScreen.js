...
const DontStopMeNowScreen = props => {
  ...
  React.useEffect(() => {
    try {
      ...
      videoV5fq6RHbRef.current?.pause();
      videoV5fq6RHbRef.current?.seekToPosition('00');
    } catch (err) {
      console.error(err);
    }
  }, [isFocused]);
  const videoV5fq6RHbRef = React.useRef();
...
export default withTheme(DontStopMeNowScreen);
